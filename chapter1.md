...
- Use `select()`, `group_by()`, `%>%` and `summarize()` to generate new weighted ratings for each restaurant while also creatng columns: `cnt = n()`, `avg = sum(stars) / cnt`,  `new = sum(weighted_stars) / sum(total_reviews)`, `dif = new - avg`
